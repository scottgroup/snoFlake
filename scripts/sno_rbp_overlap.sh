...
#SBATCH -o /home/kris98/scratch/network_analysis/network_output/slurmout/sno_rbp_overlaps_p_vals/output.%j.out
#SBATCH -e /home/kris98/scratch/network_analysis/network_output/slurmout/sno_rbp_overlaps_p_vals/output.%j.err
...
cp -r $data/snoglobe_formatted/ $SLURM_TMPDIR/
...
# snoRNA ENSEMBL ID in the 1st column
...
# need to remove redundancy in bed bfile with gene names
# if a region overlaps n genes, the region will be written n times. 
# Can use cut -f1-6 file_with_geneids.bed | sort -k1,1 -k2,3n -u > unique_regions.bed
...
tmp_outpath=$SLURM_TMPDIR/sno_rbp_overlaps_p_vals/
# create ouptut path
mkdir -p $tmp_outpath

# ouptut file
tmp_outfile=$tmp_outpath/${sno}_rbp_overlap.tsv
...
outpath=$SCRATCH/network_analysis/network_output/sno_rbp_overlaps_p_vals/
mkdir -p $outpath
